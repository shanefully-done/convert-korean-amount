...
describe("first", () => {
  ...
  test("[띄어쓰기 없이] 527000000 --> 5억2,700만원", () => {
    expect(convertKoreanAmount(527000000, { wordSpacing: false })).toBe("5억2,700만원");
  });

  test("[원 없이] 3000000 --> 3,000만", () => {
    expect(convertKoreanAmount(30000000, { withWonSuffix: false })).toBe("3,000만");
  });

  test("[콤마 없이] 527000000 --> 5억 2700만원", () => {
    expect(convertKoreanAmount(527000000, { useComma: false })).toBe("5억 2700만원");
  });

  test("[띄어쓰기/원 없이] 527000000 --> 5억2700만", () => {
    expect(convertKoreanAmount(527000000, {
      wordSpacing: false,
      withWonSuffix: false,
      useComma: false,
    })).toBe("5억2700만");
